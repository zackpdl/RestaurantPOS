...
export default function OrderScreen() {
  ...
  const handleSaveOrder = async () => {
    try {
      const orderType = type === 'dine-in' ? 'dine-in' : 'takeaway';
      const restaurant = await loadSelectedRestaurant();
      const saved = await saveOrderToStorage({
      ...
      if (!Number.isNaN(orderNumber)) {
        await setOrderStatus(orderType, orderNumber, true);
      }

      router.push(`/order-view/${saved.id}`);
    } catch (error) {
